...
//				const double s1a_d = (config.xpcg.pos - config.r_wr.x() + config.sy * s12_d - config.sx * c12_d) / l1.length;
//				const double c1a_d = sqrt(1 - s1a_d * s1a_d);
...
//
//				const double desired_py = config.sy * c12_d + config.sx * s12_d + l1.length * c1a_d + config.r_wr.y();
//				const double desired_vy = (-config.sy * s12_d + config.sx * c12_d) * vel_a - l1.length * s1a_d * desired_th1_dot;
//
//
...
//				double pye = desired_py - config.py;
//				double vye = desired_vy - vyr;
//				config.ypcg.int_pos_error += pye * dt;
//
//				const Eigen::Vector3d gain_x = { 800.0, 1000.0 ,  25.0 };
//				const Eigen::Vector3d err_x = { pxe,   config.xpcg.int_pos_error, vxe };
//				const Eigen::Vector3d gain_y = { 800.0, 1000.0 ,  25.0 };
//				const Eigen::Vector3d err_y = { pye,   config.ypcg.int_pos_error, vye };
//
//				Eigen::Vector2d torque = M * Jp.inverse() * Eigen::Vector2d(gain_x.transpose() * err_x, gain_y.transpose() * err_y)
//					- J.transpose() * omega;
//
//				j1.torque = torque[0];
//				j2.torque = torque[1];
